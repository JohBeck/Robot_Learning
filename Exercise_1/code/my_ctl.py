...
def my_ctl(ctl, q, qd, q_des, qd_des, qdd_des, q_hist, q_deshist, gravity, coriolis, M):
    ...
    if ctl == 'P':
        ...
        u = np.dot(K_P, (np.mat(q_des).T - np.mat(q).T)) + \
            np.dot(K_D, (np.mat(qd_des).T - np.mat(qd).T))
    elif ctl == 'PID':
        # u_t = K_P(q_des-q)+K_D(qd_des-qd)+K_I*all-time steps
        u = K_P * (np.mat(q_des).T - np.mat(q).T) + \
            K_D * (np.mat(qd_des).T - np.mat(qd).T) + \
            K_I * np.trapz(np.mat(q_deshist-q_hist).T)
    elif ctl == 'PD_Grav':
        ...
        qdd_ref = np.mat(qdd_des).T + \
                  K_D*(np.mat(qd_des).T-np.mat(qd).T) + \
                  K_P * (np.mat(q_des).T - np.mat(q).T)
